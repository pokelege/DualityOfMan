...
public class Player1Controls : PlayerControls
{
    ...
    public void movePlayer(Player player, float velocity = 10, float jumpPower = 10, AudioClip jumpSound = null, AudioClip punchSound = null)
    {
        ...
        if (ani.getAnimation().Equals(AnimationHack.CurrentAnimation.Punch) && ani.finished)
        {
            ...
            if (detector.punched != null)
            {
                detector.punched.GetComponent<Player>().health -= 1;
            }
            if (punchSound != null)
            {
                player.audio.PlayOneShot(punchSound);
            }
            ani.setAnimation(AnimationHack.CurrentAnimation.Stand);
        }
        Vector3 totalDirection = new Vector3();
        totalDirection += player.camera.transform.forward * Input.GetAxis("Vertical_P1");
        totalDirection += player.camera.transform.right * Input.GetAxis("Horizontal_P1");

        if (Input.GetButton("Punch_P1") && !ani.getAnimation().Equals(AnimationHack.CurrentAnimation.Punch))
        {
            ani.setAnimation(AnimationHack.CurrentAnimation.Punch, false);
        }
        totalDirection.y = 0;

        if (totalDirection.Equals(Vector3.zero))
        {
            player.playerModel.rigidbody.velocity = new Vector3(0, player.playerModel.rigidbody.velocity.y, 0);
        }
        else
        {
        ...
        if (player.collided && Input.GetButton("Jump_P1"))
        {
    ...
    public void rotateCam(Player player, float velocity = 10, float cameraDistance = 5, float cameraHeight = 3, float cameraFocusHeight = 3, float cameraSpeed = 25, float cameraMaxDistance = 10)
    {
        ...
        if (Input.GetAxis("CamHorizontal_P1") > 0)
        {
